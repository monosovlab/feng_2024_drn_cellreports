function varargout=sigstar(groups,stats,getmin,firststarp,nosort,fontsize,forceptxt,forcedminy)
    ...
    % getmin - hacky, yyf, negative stars
    % firststarp - hacky, yyf, don't show star(s) if p >= this
    ...
    %if you don't actually have groups but just want to put sigstars for
    %each bar
    if ~iscell(groups)
...
%         stats=repmat(0.05,1,length(groups));
%     end
    if nargin<3 || isempty(getmin)
        getmin=false(1,length(groups));
    end
    if nargin<4 || isempty(firststarp)
        firststarp=0.05;
    end
    if nargin<5 || isempty(nosort)
        nosort=0;
    end
    if nargin<6 || isempty(fontsize)
        fontsize=10;
    end
    if nargin<7 || isempty(forceptxt)
        forceptxt=false;
    end
    if isempty(getmin)
        getmin=false(1,length(groups));
    ...
    if ~exist('forcedminy','var') || isempty(forcedminy)
        thisY = NaN(1,length(groups));
        for ii=1:length(groups)
            thisY(ii)=findMinY(xlocs(ii,:),getmin(ii))+ternary(getmin(ii),-yd*2,yd);
        end
    else
        thisY = forcedminy*ones(1,length(groups));
    end
    for ii=1:length(groups)
        H(ii,:)=makeSignificanceBar(xlocs(ii,:),thisY(ii),stats(ii),fontsize,forceptxt,firststarp);
    end
    %-----------------------------------------------------
...
function H=makeSignificanceBar(x,y,p,fontsize,forceptxt,firststarp)
    ...
    if forceptxt
        ...
        stars = significance_text(p,firststarp);
    end
    
    if ~isempty(regexp(stars,'\*', 'once'))
        fontsize = fontsize*2;
    end
