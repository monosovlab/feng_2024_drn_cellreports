function [h] = plot_line_xint(val,varargin)
% [h] = plot_line_xint([axes handle],val,varargin)
%
...
if isa(val,'matlab.graphics.axis.Axes') || isa(val,'matlab.ui.control.UIAxes')
    assert(~isempty(varargin),'plot_line_xint was given an axes but no data to plot on it!');
    ax = val;
    val = varargin{1};
...
val = torowvector(val);
h = plot(ax,repmat(val,2,1),extent.*repmat([-1;1],1,numel(val)),varargin{:},'yliminclude','off');
