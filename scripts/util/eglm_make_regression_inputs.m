function [x,y,normalization_params,glmfun_inputs] = eglm_make_regression_inputs(xreg,yreg,oktrials,keep_offers)
...
y = yreg;
if all(isnan(yreg))
%     warning("yreg is all NaN"); % need to make yreg a vector of length n_trials
else
    if isstruct(yreg)
...
if exist('keep_offers','var')
    assert(keep_offers == 0 || keep_offers == 1 || islogical(keep_offers));
else
    keep_offers = false;
end
y = y(oktrials,:);
...
for xi = 1:numel(xreg)
    ...
    if ischar(cur.normalization)
        ...
        assert(strcmp(cur.normalization{1},'oktrials'));
